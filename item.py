...
class Item(WebsiteGenerator):
	...
	def autoname(self):
		if(self.item_code=="000"):
			self.item_code_gen()
		#frappe.msgprint(self.item_code+"Auto name",raise_exception=1)
	...
	def after_insert(self):
		frappe.db.sql("""update tabItem set item_code=%s where name=%s""",(self.name,self.name))
		if self.variant_of:
			name_variants=self.name
			name_variants_split=name_variants.split("-")
			new_name=name_variants_split[1]
			prefix_variant=self.prefix+new_name
			#frappe.msgprint(prefix_variant)
			frappe.db.sql("""update tabItem set prefix=%s where name=%s""",(prefix_variant,name_variants))
	
	def validate(self):
		super(Item, self).validate()
		#self.item_code_gen()
		#frappe.msgprint(self.item_code,raise_exception=1)
	...
	def item_code_gen(self):
		#item code
		'''
		Item Group first letter +
		Item Division Number +
		Customer Short Name +
		Product Short Name +
		package category first letter +
		Category Number +
		Subcategory Number +
		Running Number
		'''
		#group
		if(self.material_type=="Item"):
			item_group=self.item_group
			item_group_first = item_group[0]
		#division
			item_division=self.item_division
		...
		#customer
			...
			if short_cust == None:
				frappe.msgprint("Customer Short Name Not Set for <b>"+customer_name+"</b> So Please Set Customer Short Name",raise_exception=1);
			
		#product
			pro_name=self.product
			if(pro_name):
				short_product= frappe.db.get_value("Item", pro_name, "product_short_name")
			else:
				short_product=""
			if short_product == None:
				frappe.msgprint("Product Short Name Not Set for <b>"+pro_name+"</b> So Please Set Product Short Name",raise_exception=1);
			#if(!(short_product)):
			#frappe.msgprint("Enter Product Short Name for "+pro_name,raise_exception=1)
	#package
			package_category=self.package_category
			if(package_category):
				package_category_letter = package_category[0]
			else:
				package_category_letter=""
				#frappe.msgprint("Product Short Name Not Set for "+pro_name+" So Please Set Product Short Name",raise_exception=1);
		#category
			...
			if(category):
				...
				auto_number_cat="00"
		#subcategory
			subcategory=self.item_sub_category
			if(subcategory):
				#frappe.msgprint(subcategory,raise_exception=1);
		...
		#runing number
			...
			self.prefix=prefix
			self.barcode=item_code
		if(self.material_type=="Product"):
			item_group=self.item_group
			...
			self.prefix=prefix
			self.barcode=item_code
		#frappe.msgprint("this is item code check",raise_exception=1)
	...
	def customer_code_update(self):
		customer_code=self.customer_code;
		#frappe.msgprint(customer_code+"Cust Test");
		if(customer_code==""):
			customer_name=self.customer_name;
			result_cust_code=frappe.db.sql("""select customer_code from tabCustomer where customer_name=%s""",customer_name)
			self.customer_code=result_cust_code;
...
def custom_validate(doc,method):
	...
	if(material_type=="Item"):
		product=doc.product
		package_category=doc.package_category
		item_category=doc.item_category
		if(product==None):
			frappe.msgprint("Product is mandatory",raise_exception=1)
		if(package_category==None):
			frappe.msgprint("Package Category is mandatory",raise_exception=1)
		if(item_category==None):
			frappe.msgprint("Item Category is mandatory",raise_exception=1)

@frappe.whitelist()
def customer_code_load(customer_name=None):
	return frappe.db.sql("""select customer_code from tabCustomer where name=%s""",customer_name)

@frappe.whitelist()
def mpn_num_load(item_attribute_value=None):
	return frappe.db.sql("""select manufacturer_part_number from `tabItem Attribute Value` where attribute_value=%s""",item_attribute_value)


@frappe.whitelist()
def item_code_ajax_gen(material_type=None,item_group=None,item_division=None,customer_name=None,pro_name=None,package_category=None,category=None,subcategory=None):
    #item code
		'''
		Item Group first letter +
		Item Division Number +
		...
		#runing number
			...
			#self.prefix=prefix
			#self.barcode=item_code
			ret = {'item_code':item_code,'prefix':prefix}
			return ret
